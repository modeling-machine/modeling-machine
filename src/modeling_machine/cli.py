...
def options_and_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="Modeling-Machine thinks for you about the meanings.",
        epilog="Example:  modeling-machine input_text.txt > output_text.txt"
...
def run():
    """
    $ text | modeling-machine                        # Accepts text from the pipe
    $ echo "...<text>..." | modeling-machine         #

    $ modeling-machine multilogue.txt new_turn.txt    # ...or files.
    """
    ...
    try:
        ...
        syslog.openlog(
            ident="modeling-machine",
            logoption=syslog.LOG_NDELAY,
            facility=syslog.LOG_USER
        )
        # Signal (single line less than 4096 only!)
        syslog.syslog(syslog.LOG_INFO, f"modeling-machine: {utterance}.")
        syslog.closelog()
